...
class MPLC4LogsManager:
    ...
    def get_logs(self, which: str) -> tuple:
        try:
            ...
            if which == "used":
                ...
                raise SyntaxError(
                    f"передан неподдерживаемый аргумент функции - {which}"
                )
            ...
            logging.error(
                f"{self._logs_owner}: ошибка получения списка файлов: {error}"
            )
            exit(1)

    def is_limits_reached(self) -> bool:
        return (
            len(self.get_logs("all")) >= MAX_LOGS_COUNT
            or System.get_diskspace_usage() >= MAX_DISK_USAGE
        )
    ...
    def remove(self, which: str):
        try:
            ...
            if not files:
                logging.info(
                    f"{self._logs_owner}: список запрашиваемых файлов ({which}) пуст"
                )
                return
