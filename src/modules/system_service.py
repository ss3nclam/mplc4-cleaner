...
class SystemService(object):
    ...
    def isactive(self) -> bool:
        ...
        try:
            ...
            logging.error(
                f"{self.__logs_owner}: ошибка проверки статуса: {shell_error}"
            )
            sys.exit(1)
        ...
        if self.isactive():
            ...
            try:
                if self.isactive():
                    ...
                    shell_error: str = shell.stderr.strip("\n")
                    logging.warning(
                        f'{self.__logs_owner}: не удалось перезапустить - "{shell_error}", попытка принудительного запуска..'
                    )
                    ...
                    if not self.isactive():
                        raise SystemError(
                            f"не удалось перезапустить и принудительно запустить"
                        )
